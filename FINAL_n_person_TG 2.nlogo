...
turtles-own [
  reputation        ; covariate from 0 to 1 ;; Casare & Sichman, 2005
  wealth
  partnered?        ; am I partnered?
  role              ; what role am I currently playing in the TG (investor, trustee)
  partner           ; who is my partner (nobody if not partnered)
  investment        ; during a TG, the amount invested by the investor before it is multiplied
  multiplied-investment      ; during a TG, the investment received by the trustee after it is multiplied
  reciprocation     ; during a TG, the percentage of the multiplied-investment returned to the investor by the trustee  ;; Milinski, 1987
  returned-investment ; during a TG, the amount returned by the trustee to the investor
  category          ; categories: Rich and reputable, poor and reputable, rich and disreputable, poor and disreputable. Discriminant is the population average
  endowment         ; amount endowed to an investor in the TG
  multiplier        ; multiplier used in the TG for this agent
  delta             ; inverse value of how much the last reciprocation counts towards the new reputation of an agent. (delta of .90 means the last reciprocation accounts for 10% of the newly calculated reputation)
  investment-norm   ; covariate, 0 to 1, base value for how much to invest in a TG (before randomness and other effects)  ;;
  reciprocation-norm   ; covariate, 0 to 1, base value for how much to reciprocate in a TG (before randomness and other effects)  ;;
]

breed [bees bee]  ; also referred to as Group-A in the interface, arbitrarily chosen to be bees  ;;
breed [rabbits rabbit]     ; also referred to as Group-B in the interface, arbitrarily chosen to be rabbits
breed [fish a-fish]    ; also referred to as Group-C in the interface, arbitrarily chosen to be fish
...
to setup
  ...
  setup-turtles        ;setup the turtles and distribute them randomly  ;;  no reference for design choice of random initial distribution of agents

  reset-ticks         ; reset "time"

  if (initial-reputation-random? = true) [          ;;  no reference for design choice of random initial reputation

    ask turtles [set reputation random-float 1]]
end
...
to make-turtles                 ; creates 3 (or more...) different breeds of agents with different      ;;  Van Baar et al 2020
  create-bees num-turtles-A                ; bees are the default shape for group A, but may be changed in the interface
  ask bees [set shape shape-A]
  ask bees [set reputation initial-reputation-A]
  ask bees [set wealth initial-wealth-A]
  ask bees [set endowment endowment-A]
  ask bees [set delta delta-A]
  ask bees [set multiplier multiplier-A]
  ask bees [set investment-norm investment-norm-A]
  ask bees [set reciprocation-norm reciprocation-norm-A]
  create-rabbits num-turtles-B                ; rabbits are the default shape for group B, but may be changed in the interface
  ask rabbits [set shape shape-B]
  ask rabbits [set reputation initial-reputation-B]
  ask rabbits [set wealth initial-wealth-B]
  ask rabbits [set endowment endowment-B]
  ask rabbits [set delta delta-B]
  ask rabbits [set multiplier multiplier-B]
  ask rabbits [set investment-norm investment-norm-B]
  ask rabbits [set reciprocation-norm reciprocation-norm-B]
  create-fish num-turtles-C                ; fish are the default shape for group C, but may be changed in the interface
  ask fish [set shape shape-C]
  ask fish [set reputation initial-reputation-C]
  ask fish [set wealth initial-wealth-C]
  ask fish [set endowment endowment-C]
  ask fish [set delta delta-C]
  ask fish [set multiplier multiplier-C]
  ask fish [set investment-norm investment-norm-C]
  ask fish [set reciprocation-norm reciprocation-norm-C]
...
to setup-common-variables
  ask turtles [
    ...
    set wealth 100             ;; no reference for design choice of initial wealth = 100

    set partnered? false

    set partner nobody                ;; no reference for design choice of initially no partners
...
to go
  ...
  if (die-if-broke? = true) [            ;; survival of the fittest, evolutionary instrument
  ...
  if (play-TG? = true) [
    ...
    ask partnered-turtles [ invest ]

    ask partnered-turtles [ reciprocation-decision ]

    ask partnered-turtles [ return-money ]

    ask partnered-turtles [ set size (1 + 2 * (((wealth - (mean [wealth] of turtles)) / (wealth + .01)))) ]

    ask turtles [

      if (reputation >= 0.5) [

        set color (63 - reputation * 10)]

      if (reputation < 0.5) [

        set color (reputation * 10 + 13)]
  ]]
  tick
end
...
to partner-up ;turtle procedure
  if (not partnered?) [               ; make sure still not partnered
    ...
    if partner != nobody [              ; if successful grabbing a partner, partner up
      ...
      set wealth (wealth - cost-of-life)   ; pay the cost-of-life for partnering up

      set heading 270                   ; face partner

      set role "investor"               ; IMPORTANT: this is not a truly random procedure to determine who is investor
...
to invest
  if (role = "investor" ) [
    ...
    set investment ((beta * [reputation] of partner + (1 - beta) * investment-norm)  )    ; invest based on investment norm and rep of partner proportinate to beta (beta is the reputation weight)

    set investment ((investment * (1 - phi) + random-float 1 * phi) * endowment)     ; adding randomness proportionate to phi

    set wealth (wealth + endowment - investment)
  ]
end

to reciprocation-decision
  if (role = "trustee") [

    set multiplied-investment ([investment] of partner * multiplier)

    set reciprocation (beta * [reputation] of partner + (1 - beta) * reciprocation-norm)      ; invest based on investment norm and rep of partner proportinate to beta (beta is the reputation weight)

    set reciprocation (reciprocation * (1 - phi-r) + random-float 1 * phi-r)     ; adding randomness proportionate to phi

    set returned-investment (multiplied-investment * reciprocation)

    set wealth (wealth + multiplied-investment - returned-investment)

    set reputation (reputation * (1 - delta) + reciprocation * delta )
]
end
...
to return-money
  if (role = "investor") [
    ...
    set returned-investment [returned-investment] of partner

    set wealth (wealth + returned-investment)

    set games-played (games-played + 1)
]
end

to update-category-plot
  set-current-plot "Rich and reputable?  -  Category plot"
  clear-plot
  ask turtles [
    if (wealth > mean [wealth] of turtles) [                       ; rich means over average wealth
      if (reputation > mean [reputation] of turtles) [             ; reputable means over average reputation
        set category "rich-and-reputable"]
      if (reputation <= mean [reputation] of turtles) [
        set category "rich-and-disreputable"]
    ]
    if (wealth <= mean [wealth] of turtles) [
      if (reputation > mean [reputation] of turtles) [
        set category "poor-and-reputable"]
      if (reputation <= mean [reputation] of turtles) [
        set category "poor-and-disreputable"]]
  ]
  let counts table:counts [ category ] of turtles
...
"" ""
PENS
"wealth" 1.0 0 -16777216 true "" "plot [wealth] of turtle 0"
...
"" ""
PENS
"reputation " 1.0 0 -16777216 true "" "plot [reputation] of turtle 0"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [wealth] of turtle 1"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [reputation] of turtle 1"
...
true
false
"set-plot-y-range 0 count bees\nset-histogram-num-bars 100\nset-plot-pen-mode 1" ""
PENS
"bees" 1.0 0 -7171555 true "" "histogram [wealth] of bees"
...
PENS
"bees" 1.0 0 -7171555 true "" "histogram [reputation] of bees"
...
1884
808
Rich and reputable?  -  Category plot
NIL
NIL
0.0
10.0
...
"" ""
PENS
"poor and disreputable" 1.0 0 -8053223 true "" "plot count turtles with [category = \"poor-and-disreputable\"]"
"poor and reputable" 1.0 0 -12087248 true "" "plot count turtles with [category = \"poor-and-reputable\"]"
"rich and disreputable" 1.0 0 -13791810 true "" "plot count turtles with [category = \"rich-and-disreputable\"]"
"rich and reputable" 1.0 0 -8630108 true "" "plot count turtles with [category = \"rich-and-reputable\"]"
...
55
total N turtles
num-turtles-A + num-turtles-B + num-turtles-C
17
1
11
...
true
false
"set-plot-x-range 0 1000\nset-plot-y-range 0 count rabbits\nset-histogram-num-bars 100\nset-plot-pen-mode 1" ""
PENS
"rabbits" 1.0 0 -9276814 true "" "histogram [wealth] of rabbits"
...
PENS
"default" 1.0 0 -14730904 true "" "histogram [wealth] of fish"
...
"set-plot-x-range 0 1\nset-plot-y-range 0 count rabbits\nset-histogram-num-bars 100\nset-plot-pen-mode 1" ""
PENS
"default" 1.0 0 -7500403 true "" "histogram [reputation] of rabbits"
...
"set-plot-x-range 0 1\nset-plot-y-range 0 count fish\nset-histogram-num-bars 100\nset-plot-pen-mode 1" ""
PENS
"default" 1.0 0 -14730904 true "" "histogram [reputation] of fish"
...
"" ""
PENS
"bees wealth" 1.0 0 -1184463 true "" "plot mean [wealth] of bees"
"rabbits wealth" 1.0 0 -7500403 true "" "plot mean [wealth] of rabbits"
"fish wealth" 1.0 0 -14730904 true "" "plot mean [wealth] of fish"
...
"" ""
PENS
"bees reputation" 1.0 0 -1184463 true "" "plot mean [reputation] of bees"
"rabbits reputation" 1.0 0 -5987164 true "" "plot mean [reputation] of rabbits"
"fish reputation" 1.0 0 -13345367 true "" "plot mean [reputation] of fish"
...
"fish" 1.0 0 -13345367 true "" "plot count fish"
...
670
137
mean wealth gain per tick
( (mean [wealth] of turtles - 100) / ticks )
17
1
11
...
1865
53
trustee-reputation-threshold-active?
trustee-reputation-threshold-active?
1
1
-1000
...
1598
55
investor-reputation-threshold-active?
investor-reputation-threshold-active?
1
1
-1000
...
1601
119
recip-norm-for-disreputable-investors
recip-norm-for-disreputable-investors
0
1
0.0
...
1872
119
inv-norm-for-disreputable-trustees
inv-norm-for-disreputable-trustees
0
1
0.01
...
"" ""
PENS
"bees" 1.0 0 -1184463 true "" "plot ( (mean [wealth] of bees - 100) / (ticks + .01) )"
"rabbits" 1.0 0 -7500403 true "" "plot ( (mean [wealth] of rabbits - 100) / (ticks + .01) )"
"fish" 1.0 0 -13791810 true "" "plot ( (mean [wealth] of fish - 100) / (ticks + .01) )"
...
<experiments>
  <experiment name="experiment3" repetitions="10" runMetricsEveryStep="false">
    ...
    <go>go</go>
    <timeLimit steps="333"/>
    <metric>[reputation] of turtles</metric>
    <metric>[wealth] of turtles</metric>
    <metric>count turtles</metric>
    <steppedValueSet variable="BIG-REPUTATION-SLIDER" first="0" step="0.25" last="1"/>
  </experiment>
  <experiment name="experiment3" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>count turtles</metric>
    <steppedValueSet variable="BIG-REPUTATION-SLIDER" first="0" step="0.25" last="1"/>
    <steppedValueSet variable="reciprocation-norm-A" first="0.2" step="0.1" last="0.8"/>
  </experiment>
  <experiment name="experiment4" repetitions="1" runMetricsEveryStep="true">
    ...
    <timeLimit steps="200"/>
    <metric>count turtles</metric>
    <metric>delta-A</metric>
    <metric>reciprocation-norm-A</metric>
    <metric>cost-of-life</metric>
    <metric>beta</metric>
    <enumeratedValueSet variable="delta-A">
      <value value="0.7"/>
      <value value="0.8"/>
      <value value="0.9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-turtles-A">
      <value value="70"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="phi-r">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="die-if-broke?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="play-TG?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reciprocation-norm-A">
      <value value="0.25"/>
      <value value="0.5"/>
      <value value="0.75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investor-reputation-threshold-active?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investors-reputation-relevant?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="cost-of-life">
      <value value="11"/>
      <value value="12"/>
      <value value="13"/>
      <value value="14"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-wealth-A">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="endowment-A">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-reputation-random?">
      <value value="false"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="initial-reputation-A">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investment-norm-A">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="trustee-reputation-threshold-active?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reciprocation-randomness?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="multiplier-A">
      <value value="4"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wealth-threshold-active?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investment-randomness?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="shape-A">
      <value value="&quot;bee&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment5_InvNorm75" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="300"/>
    <metric>count turtles</metric>
    ...
    <metric>cost-of-life</metric>
    <metric>beta</metric>
    <enumeratedValueSet variable="shape-B">
      <value value="&quot;rabbit&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="shape-C">
      <value value="&quot;fish&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="delta-A">
      <value value="0.85"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="delta-B">
      <value value="0.85"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-turtles-A">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="trustee-reputation-threshold">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investor-reputation-threshold">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="delta-C">
      <value value="0.85"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="phi-r">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="endowment-B">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-wealth-C">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-turtles-B">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="endowment-C">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="die-if-broke?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="recip-norm-for-disreputable-investors">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="play-TG?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-reputation-B">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-reputation-C">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reciprocation-norm-A">
      <value value="0.3"/>
    ...
    <enumeratedValueSet variable="reciprocation-norm-B">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wealth-threshold">
      <value value="1"/>
    </enumeratedValueSet>
    <steppedValueSet variable="cost-of-life" first="12.5" step="0.5" last="14"/>
    <enumeratedValueSet variable="reciprocation-norm-C">
      <value value="0.71"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-wealth-A">
      <value value="100"/>
    ...
    <enumeratedValueSet variable="initial-wealth-B">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="multiplier-C">
      <value value="4"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-reputation-random?">
      <value value="false"/>
    </enumeratedValueSet>
    <steppedValueSet variable="beta" first="0" step="0.2" last="1"/>
    <enumeratedValueSet variable="phi">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="inv-norm-for-disreputable-trustees">
      <value value="0.01"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-reputation-A">
      <value value="0.5"/>
    ...
    <enumeratedValueSet variable="investment-norm-B">
      <value value="0.75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investment-norm-C">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reciprocation-randomness?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-turtles-C">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="trustee-reputation-threshold-active?">
      <value value="false"/>
    ...
    <enumeratedValueSet variable="multiplier-B">
      <value value="4"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="investment-randomness?">
      <value value="true"/>
  ...
  <experiment name="experiment6_3" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="300"/>
    <metric>count turtles</metric>
  ...
  <experiment name="experiment8" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count turtles</metric>
  ...
  <experiment name="experiment7" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="300"/>
    <metric>count turtles</metric>
